...
export const easing = {
  ...
  easeInOutBounce: (x) =>
    x < 0.5 ? (1 - easing.easeOutBounce(1 - 2 * x)) / 2 : (1 + easing.easeOutBounce(2 * x - 1)) / 2,
};
 ...
 * @return {Pattern} a signal pattern
 */
export const { linear } = register(['linear'], (pat) => signal(sampleFactory(pat, 'linear')));
 ...
 * @return {Pattern} a signal pattern
 */
export const { ease, easeinout, easeInOut } = register(['ease', 'easeinout', 'easeInOut'], (ease, pat) =>
  signal(sampleFactory(pat, `easeInOut${camelize(ease)}`)),
);
 ...
 * @return {Pattern} a signal pattern
 */
export const { easeIn, easein } = register(['easeIn', 'easein'], (ease, pat) =>
  signal(sampleFactory(pat, `easeIn${camelize(ease)}`)),
);
 ...
 * @return {Pattern} a signal pattern
 */
export const { easeOut, easeout } = register(['easeOut', 'easeout'], (ease, pat) =>
  signal(sampleFactory(pat, `easeOut${camelize(ease)}`)),
);
